...
from debate_decision_system.analytics import quality_report
...
def _render_turn(turn: Turn) -> None:
    if turn["role"] == "huddle":
        with st.expander(f"Huddle · {turn['name']}", icon=":material/groups:"):
            st.markdown(turn["content"])
        return
    avatar = AVATARS.get(turn["role"], ":material/chat:")
    with st.chat_message(turn["role"], avatar=avatar):
        if " / " not in turn["name"] and turn["role"] == "debater":
            debate = st.session_state.get("debate") or {}
            seat = next(
                (s for s in (debate.get("debaters") or []) if s.get("name") == turn["name"]),
                {},
            )
            if seat.get("kind") == "team":
                st.badge("team", icon=":material/groups:", color="violet")
        st.markdown(f"**{turn['name']}**")
        st.markdown(turn["content"])


def _read_uploads(files: list[object]) -> list[Document]:
    docs: list[Document] = []
...
st.caption("Decisions save to local SQLite. Open Decision history to search and resume.")

with st.sidebar:
    local_only = st.toggle("Fully local", value=False, help="Force every seat onto Ollama.")
    grounding = st.segmented_control(
        "Knowledge",
        ["open", "grounded"],
        default="open",
        required=True,
        key="grounding",
        width="stretch",
        help="Grounded: documents + calculator/code only. Open: also web and Wikipedia.",
    )
    ...
    with st.expander("Seats", expanded=True):
        for i in range(debater_count):
            ...
            if kind == "team":
                team_name = st.selectbox("Team", team_names, key=f"team_{i}")
                n_members = st.slider(
                    "Members",
                    config.MIN_TEAM_MEMBERS,
                    config.MAX_TEAM_MEMBERS,
                    3,
                    key=f"team_n_{i}",
                )
                defaults = members_from_template(str(team_name), default_provider, default_model)
                members = []
                for j in range(n_members):
                    persona = st.selectbox(
                        f"Member {j + 1}",
                        PERSONA_NAMES,
                        index=PERSONA_NAMES.index(defaults[j]["name"])
                        if j < len(defaults) and defaults[j]["name"] in PERSONA_NAMES
                        else j % len(PERSONA_NAMES),
                        key=f"member_{i}_{j}",
                    )
                    m_provider, m_model = _pick_provider_model(
                        f"Member {j + 1}", f"m_{i}_{j}", default_provider, allowed
                    )
                    if not m_model:
                        m_provider, m_model = default_provider, default_model
                    leader = st.toggle("Leader", value=j == 0, key=f"lead_{i}_{j}")
                    persona_obj = next(p for p in PERSONAS if p.name == persona)
                    members.append(
                        {
                            "name": persona_obj.name,
                            "style": persona_obj.style,
                            "instructions": persona_obj.instructions,
                            "provider": m_provider,
                            "model": m_model,
                            "is_leader": leader,
                        }
                    )
                    used_providers.add(m_provider)
                seats.append({"name": team_name, "kind": "team", "members": members})
                persona_names.append(str(team_name))
                seat_models.append((default_provider, default_model))
            else:
                persona = st.selectbox(
                    "Persona",
                    PERSONA_NAMES,
...
if verdict:
    with st.container(border=True):
        ...
        quality = quality_report(st.session_state.debate)
        with st.expander("Debate quality report", icon=":material/analytics:"):
