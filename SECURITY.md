...
The Streamlit UI, SQLite file (`data/decisions.db`), and LanceDB store (`data/lancedb/`) run on **your** machine. There is no hosted multi-user service and no project-operated API. You supply every credential (`.env` / OS environment). Anything you upload or debate is **your** responsibility — see [DISCLAIMER.md](DISCLAIMER.md).
...
Use GitHub **Security Advisories** on this repository (private report). Do not open a public issue for a vulnerability.
